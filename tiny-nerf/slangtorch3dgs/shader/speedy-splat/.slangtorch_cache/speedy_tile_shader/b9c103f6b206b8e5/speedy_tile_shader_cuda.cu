...
__device__ int unpackAnyValue4_0(AnyValue4 _S1)
{
    return (slang_bit_cast<int>(_S1.field0_0));
}

__device__ AnyValue4 packAnyValue4_0(float _S2)
{
    AnyValue4 _S3;
    (&_S3)->field0_0 = (slang_bit_cast<uint>(_S2));
    return _S3;
}


#line 19 "D:/A_study/nerf3dgs/gaussian-splatting(test)/slangRenderers/shader/speedy-splat/speedy_tile_shader.slang"
__global__ void __kernel__generate_keys(TensorView xyz_vs_0, TensorView rect_tile_space_0, TensorView index_buffer_offset_0, TensorView out_unsorted_keys_0, TensorView out_unsorted_gauss_idx_0, uint grid_height_0, uint grid_width_0)
{

#line 27
    int globalIdx_0 = int(((blockIdx)).x * ((blockDim)).x + ((threadIdx)).x);

    uint _S4 = uint(globalIdx_0);

#line 29
    uint _S5 = ((xyz_vs_0).sizes[(0U)]);
...
#line 30
    ...
    float _S6 = ((xyz_vs_0).load<float>((_S4), (0U)));

#line 32
    float _S7 = ((xyz_vs_0).load<float>((_S4), (1U)));

#line 32
    float _S8 = ((xyz_vs_0).load<float>((_S4), (2U)));

#line 32
    int offset_0;

#line 40
    if(globalIdx_0 == int(0))
    {
...
#line 40
    ...
    {
        int _S9 = ((index_buffer_offset_0).load<int>((uint(globalIdx_0 - int(1)))));
...
#line 45
    int _S10 = ((rect_tile_space_0).load<int>((_S4), (0U)));
    int _S11 = ((rect_tile_space_0).load<int>((_S4), (1U)));
    int _S12 = ((rect_tile_space_0).load<int>((_S4), (2U)));
    int _S13 = ((rect_tile_space_0).load<int>((_S4), (3U)));

#line 48
    int y_0 = _S11;

#line 56
    ulonglong _S14 = ulonglong(unpackAnyValue4_0(packAnyValue4_0(_S8)));
...
#line 52
            ...
            (out_unsorted_keys_0).store<longlong>((uint(offset_0)), (longlong(ulonglong(uint(y_0) * grid_width_0 + uint(x_0)) << int(32) | _S14)));
            (out_unsorted_gauss_idx_0).store<int>((uint(offset_0)), (globalIdx_0));
            int _S15 = offset_0 + int(1);
...
__global__ void __kernel__compute_tile_ranges(TensorView sorted_keys_0, TensorView out_tile_ranges_0)
{
    ...
    int globalIdx_1 = int(((blockIdx)).x * ((blockDim)).x + ((threadIdx)).x);
