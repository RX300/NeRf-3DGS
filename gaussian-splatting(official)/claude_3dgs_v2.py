...
from scene.gaussian_model_tiny import GaussianModelTiny
...
class GaussianSplatting:
    ...
    def initialize_gaussians(self):
        # self.gaussians = GaussianModelTiny(self.sh_degree)
        # self.gaussians.create_from_pcd(self.pcd, 0, 1.0)
        # self.means = self.gaussians._xyz
        # self.scales = self.gaussians._scaling
        # self.rotations = self.gaussians._rotation
        # self.opacities = self.gaussians._opacity
        # self.features_dc = self.gaussians._features_dc
        # self.features_rest = self.gaussians._features_rest
        # print(f"features_dc.shape=>{self.features_dc.shape}")
        # print(f"features_rest.shape=>{self.features_rest.shape}")
        # parser = ArgumentParser(description="Training script parameters")
        # self.op = OptimizationParams(parser)
        ...
        # self.gaussians.training_setup(self.op.extract(args),self.learning_rate)     
    ...
    def train(self, num_epochs=100, batch_size=4096):
...
if __name__ == "__main__":
    ...
    # Train the model
    losses = model.train(num_epochs=1000, batch_size=4096)
