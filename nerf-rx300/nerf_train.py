...
def create_nerf(args, device)-> tuple[dict[str, any], dict[str, any], int, list[torch.nn.Parameter], torch.optim.Optimizer]:
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    参数:
        raw: [num_rays, num_samples along ray, 4]. 模型的预测结果。
            - 最后一维通常包含RGB和密度： [R, G, B, sigma]。
        z_vals: [num_rays, num_samples along ray]. 积分次数。
            - 这些是沿射线采样点的距离。
        rays_d: [num_rays, 3]. 每条射线的方向。
            - 射线的方向向量。
        raw_noise_std: float. 添加到原始密度的噪声标准差。
            - 用于正则化密度预测。
        white_bkgd: bool. 如果为True，假设背景为白色。
            - 修改最终的RGB颜色，添加背景颜色。
        pytest: bool. 如果为True，使用确定性的噪声用于测试。
            - 确保测试期间的可重复性。

    返回:
        rgb_map: [num_rays, 3]. 每条射线的估计RGB颜色。
        disp_map: [num_rays]. 视差图。深度图的倒数。
        acc_map: [num_rays]. 每条射线上的权重总和。
        weights: [num_rays, num_samples]. 分配给每个采样点的权重。
        depth_map: [num_rays]. 每条射线的估计物体距离。
    """

    # 将原始密度值转换为alpha（不透明度）值。
    # 通过应用激活函数确保密度值为非负。
    # 使用公式 1 - exp(-sigma * delta) 将密度和距离转换为不透明度（alpha），表示光线在该点被吸收或散射的概率。
    ...
    # 每个采样点的RGB值乘以其对应的权重，然后在采样点维度上求和，得到每条射线的最终颜色。
...
def render_rays(ray_batch,
    ...
    ray_batch：形状为 [batch_size, ...] 的数组。包含沿光线采样所需的所有信息，包括：光线起点、光线方向、最小距离、最大距离，以及单位大小的视向量。
    network_fn：函数。用于预测每个空间点的 RGB 和密度的模型。
    network_query_fn：用于将查询传递给 network_fn 的函数。
    N_samples：整数。每条光线沿光线采样的次数。
    retraw：布尔值。如果为 True，则包括模型的原始未处理预测。
    lindisp：布尔值。如果为 True，则沿逆深度（而非深度）进行线性采样。
    perturb：浮动值，0 或 1。如果非零，每条光线将在时间上以分层随机的点进行采样。
    N_importance：整数。每条光线沿光线采样的额外次数。这些样本仅传递给 network_fine。
    network_fine：具有与 network_fn 相同规格的“精细”网络。
    white_bkgd：布尔值。如果为 True，假设背景为白色。
    raw_noise_std：...
    verbose：布尔值。如果为 True，则打印更多调试信息。
    返回：

    rgb_map：[num_rays, 3]。光线的 RGB 颜色估计值，来自精细模型。
    disp_map：[num_rays]。视差图。深度的倒数。
    acc_map：[num_rays]。每条光线的累计不透明度，来自精细模型。
    raw：[num_rays, num_samples, 4]。模型的原始预测。
    rgb0：参见 rgb_map。粗略模型的输出。
    disp0：参见 disp_map。粗略模型的输出。
    acc0：参见 acc_map。粗略模型的输出。
    z_std：[num_rays]。每条光线沿射线的标准差。
    """
...
def train(args, device):
    ...
    if args.dataset_type == 'blender':
        ...
        near = 2.
        far = 6.
        # 根据 args.white_bkgd 是否为 True，选择对图像进行背景处理
        #（如果是白色背景，则图像的颜色信息会与 alpha 通道结合）。
        if args.white_bkgd:
    ...
    # 将训练配置（args）写入一个文件 args.txt 中，以便之后复现训练环境。
    basedir = args.basedir
    expname = args.expname
    ...
    # 如果启用批处理，代码会将射线和图像数据合并，并对数据进行洗牌，确保每次训练时输入数据是随机的。
    if use_batching:
